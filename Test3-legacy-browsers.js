...
async function updateInfo() {
  ...
  psychoJS.experiment.dataFileName = (("." + "/") + `data/${expInfo["date"]}`);
...
async function experimentInit() {
  ...
  text_3 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_3',
    text: 'Kösz!\n\nMentés folyamatban, kérjük megvárni! ...',
    font: 'Open Sans',
    units: undefined, 
...
function FirstRoutineBegin(snapshot) {
  return async function () {
    ...
    FirstComponents = [];
    FirstComponents.push(text);
    FirstComponents.push(key_resp);
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials.forEach(function() {
      ...
      trialsLoopScheduler.add(SecondRoutineBegin(snapshot));
      trialsLoopScheduler.add(SecondRoutineEachFrame());
      trialsLoopScheduler.add(SecondRoutineEnd(snapshot));
...
function SecondRoutineBegin(snapshot) {
  return async function () {
    ...
    // keep track of which components have finished
    SecondComponents = [];
    SecondComponents.push(text_2);
    SecondComponents.push(key_resp_2);
    
    SecondComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function GoodbyeRoutineBegin(snapshot) {
  return async function () {
    ...
    // Concert data object to CSV
    let data = [Object.keys(dataObj[0])].contact(dataObj).map(it => {
    	return Object.values(it).toString()
    }).join('\n')
    
    // Send data ...
    console.log('Saving data...');
    fetch('https://github.com/gzugiz/FirstTest/tree/main/data', {
    	method: 'POST',
    	headers: {
